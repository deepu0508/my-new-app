...
export default function TextForm(props) {
    ...
    function toHandleCopy() {
        ...
        if (text.value !== '') {
            ...
            navigator.clipboard.writeText(text.value) // text copy
    ...
    async function toPasteText() {
        if (await navigator.clipboard.readText() === `props.showAlert("Text has been not present",'warning')`) {
        ...
        <div >
            <div className="container" style={{ color: props.mode === 'dark' ? 'light' : '#343a40' }}>
                ...
                <div className="container">
                    ...
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toSmall}>Convert to Smallcase</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toClear}>Clear Text</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toHandleCopy}>Text Copy</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toPasteText}>Text Paste</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toReverse}>Convert to Reverse Order</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toSeakOut}>Read Out Text</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toEmailFind}>Email Finder</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toFindNumber}>Number Finder</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toPanctuation}>Punctuation Remover</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toWrodsArray}>Convert Words Array</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toLocalStorage}>Store Data</button>
                    ...
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toBold}>Text Bold</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toIterative}>Text Iterative</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toIterativeBold}>Text Iterative & Bold</button>
                    <button className="btn btn-cls cls btn-primary mb-3 mx-3 mt-3" onClick={toRemoveExtraSpace}>Remove Extra Space Remove</button>
                </div>
            </div>
            <div className={`container text-${props.mode === 'dark' ? 'light' : 'dark'}`} style={{ transition: '1s' }}>
                <h2>Your Text Summary</h2>
                <p>{text.length} Characters <br /> {(text === '' && text.split(/[ ]+/).length === 1) ? 0 : text.endsWith(' ') ? text.split(/[ ]+/).length - 1 : text.split(/[ ]+/).length} Words <br />{text.split("\n").length - 1} Lines</p>
                <p>{0.008 * ((text === '' || text.split(/[ ]+/).length === 1) ? 0 : text.split(/[ ]+/).length - 1)} Minutes Read</p>
                <p id="email"></p>
                <p id="number"></p>
                <p id="bold"></p>
                <textarea disabled className={`form-control my-3 text text-${props.mode === 'dark' ? 'light' : 'dark'}`} style={{ backgroundColor: props.mode === 'dark' ? '#343a40' : 'white', transition: '1s' }} rows="4" id='words'></textarea>
                <h3>Preview</h3>
